...
export const allowance = async (owner, spender, provider) => {
    const contract = getContract(TOKEN_ADDRESS, TOKEN_ABI, provider)
    const currentAllowance = await contract.allowance(owner, spender)
    return currentAllowance
}

export const increaseAllowance = async (spender, addedValue, signer) => {
    const contract = getContract(TOKEN_ADDRESS, TOKEN_ABI, signer)
    const increaseAllowanceResponseBody = await contract.increaseAllowance(spender, addedValue)
    return increaseAllowanceResponseBody
}
